...
class Dice extends Command
{
    public function trigger(Message $message, Collection $args = null)
    {
        ...
        if ($sides < 2) {
            return $message->reply("You can not roll dice with 1 side, what are you, stupid?");
        } else if ($dice < 1) {
            return $message->reply("You can not roll zero die, what are you, stupid?");
        }
        ...
        return $message->reply($response)->then(function (Message $message) use ($dice, $response, $result) {
            return $message->client->addTimer(random_int($dice, $dice * 3), function () use ($message, $response, $result) {
                $response = Str::replaceFirst('roll', 'rolled', $response);
